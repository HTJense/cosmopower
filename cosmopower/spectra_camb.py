...
def initialize(parser: YAMLParser, extra_args: dict = {}) -> dict:
    ...
    for quantity in parser.quantities:
        ...
        if parser.modes_label(quantity) == "z":
            ...
            extra_args["redshifts"] = \
                np.sort(np.unique(np.concatenate((z1, z2), 0)))
...
def get_camb_derived(pars: camb.CAMBparams, results: camb.CAMBdata,
...
def get_camb_Hubble(pars: camb.CAMBparams, results: camb.CAMBdata,
                    redshifts: Sequence[float]) -> np.ndarray:
    return results.hubble_parameter(redshifts)  # km/s/Mpc units


def get_camb_H(pars: camb.CAMBparams, results: camb.CAMBdata,
               redshifts: Sequence[float]) -> np.ndarray:
    return results.h_of_z(redshifts)  # 1/Mpc units


def get_camb_sigma8(pars: camb.CAMBparams, results: camb.CAMBdata,
                    redshifts: Sequence[float]) -> np.ndarray:
...
def get_camb_angular_diameter_distance(pars: camb.CAMBparams,
...
def get_camb_cmb_power_spectra(pars: camb.CAMBparams, results: camb.CAMBdata,
                               ls: np.ndarray, spectra: str) -> np.ndarray:
...
def get_camb_linear_matter_power(camb: ModuleType, pars: camb.CAMBparams,
                                 results: camb.CAMBdata, k: np.ndarray,
                                 z: np.ndarray, kmax: float) -> np.ndarray:
...
def get_camb_nonlinear_matter_power(camb: ModuleType, pars: camb.CAMBparams,
                                    results: camb.CAMBdata, k: np.ndarray,
                                    z: np.ndarray, kmax: float) -> np.ndarray:
...
def get_camb_nonlinear_matter_power_boost(camb: ModuleType,
                                          pars: camb.CAMBparams,
                                          results: camb.CAMBdata,
                                          k: np.ndarray, z: np.ndarray,
                                          kmax: float) -> np.ndarray:
...
def get_spectra(parser: YAMLParser, state: dict, args: dict = {},
    ...
    try:
        state["params"] = camb.set_params(cp=state["params"],
                                          **translate_camb_params(**args))
    ...
    for quantity in quantities:
        ...
        if qpath[0] == "derived":
            ...
            # C_ells
            state[quantity] = \
                get_camb_cmb_power_spectra(state["params"], state["results"],
                                           state[quantity + ".modes"],
                                           qpath[1])
        elif qpath[0] == "Pk" and qpath[1] == "lin":
            # Linear matter power
            state[quantity] = \
                get_camb_linear_matter_power(camb, state["params"],
                                             state["results"],
                                             state[quantity + ".modes"],
                                             np.asarray([args.get("z", 0.0)]),
                                             extra_args.get("kmax", 1.0))
        elif qpath[0] == "Pk" and qpath[1] == "nonlin":
            # Non-linear matter power
            state[quantity] = get_camb_nonlinear_matter_power(
                camb, state["params"], state["results"],
                state[quantity + ".modes"], np.asarray([args.get("z", 0.0)]),
                extra_args.get("kmax", 1.0)
            )
        elif qpath[0] == "Pk" and qpath[1] == "nlboost":
            # Non-linear matter power
            state[quantity] = get_camb_nonlinear_matter_power_boost(
                camb, state["params"], state["results"],
                state[quantity + ".modes"], np.asarray([args.get("z", 0.0)]),
                extra_args.get("kmax", 1.0)
            )
            ...
            state[quantity] = get_camb_Hubble(
...
def translate_camb_params(**kwargs) -> dict:
    """
    TODO: Docstring.
    """
    args = {}
